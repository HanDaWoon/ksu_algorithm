<script lang="ts">
	...
	export let run_id: number;

	class OutputLine {
		constructor(msg: string, color = 'white', direction = '>>', id = null) {
			this.id = id;
			this.msg = msg;
			this.color = color;
	...
	let output_lines: any[] = []; // list of OutputLine
	let output_html = '>> 실행 버튼을 눌려주세요';
	let input_text = '';
	let clearIntervalToken: string | number | NodeJS.Timeout | null | undefined = null;
	...
	function poll_run(run_id: string | number | null) {
			...
			customFetch({
				...
				.then((res) => {
					...
					if (res.output != null && !is_started) {
						is_started = true;
						output_lines.push(new OutputLine('프로그램 시작됨', '#9ab2ff', '>>'));
					}
					...
					if (res.output != null) {
						for (let rcv_line of res.output) {
							...
							for (let i = 0; i < output_lines.length; i++) {
								if (output_lines[i].id != null && output_lines[i].id == rcv_line.id) {
							...
							if (found) continue;
							output_lines.push(new OutputLine(rcv_line.data, 'white', '>>', rcv_line.id));
						}
					}
					...
					if (res.error != null && res.error != '') {
						...
						if (idx != -1) {
							...
							output_lines.push(new OutputLine(res.error, 'red', '>> 오류 발생:', 'error'));
						}
					...
					if (res.status == 'done') {
						output_lines.push(new OutputLine('프로그램 종료됨', '#9acd32', '>>'));
						clearInterval(localIntervalToken);
					}
	...
	function escapeHtml(str: string) {
		...
		return str.replace(/[&<>"']/g, function (m: string | number) {
			return map[m];
		});
	}
	...
	function printOuput(outputLines) {
		...
		for (let line of outputLines) {
			output_html_builder += `<div class="output-line"><span class="output-line-direction pr-1">${
				line.direction
			}</span><span class="output-line-msg" style="color: ${line.color}">${escapeHtml(
				line.msg
			)}</span></div>`;
		}
